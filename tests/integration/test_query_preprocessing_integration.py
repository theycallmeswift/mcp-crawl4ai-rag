...
class TestQueryPreprocessingIntegration:
    ...
    async def test_rag_query_with_preprocessing_enabled(
        ...
        # Setup
        with SupabaseMocker() as supabase_mocker, OpenAIMocker() as openai_mocker:
            ...
            mock_mcp_context.request_context.lifespan_context.supabase_client = (
            ...
            with patch("openai.chat.completions.create") as mock_openai:
                ...
                with patch.dict(
                    ...
                    assert (
                        data["query_preprocessing"]["semantic_query"]
                        == preprocessing_response["semantic_query"]
                    )
                    assert (
                        data["query_preprocessing"]["keywords"]
                        == preprocessing_response["keywords"]
                    )
    ...
    @pytest.mark.asyncio
    async def test_rag_query_preprocessing_disabled(self, test_env, mock_mcp_context):
        ...
        with SupabaseMocker() as supabase_mocker, OpenAIMocker() as openai_mocker:
            ...
            with patch("openai.chat.completions.create") as mock_openai:
                ...
                with patch.dict(
                    ...
                    # Note: search_breadth is not included in the actual response
                    assert data["search_mode"] == "hybrid"
        ...
        with SupabaseMocker() as supabase_mocker, OpenAIMocker() as openai_mocker:
            ...
            with patch("openai.chat.completions.create") as mock_openai:
                ...
                with patch.dict(
                    ...
                    result = await perform_rag_query(
                        mock_mcp_context, query="authentication handler", match_count=5
                    )
        ...
        with SupabaseMocker() as supabase_mocker, OpenAIMocker() as openai_mocker:
            ...
            with patch("openai.chat.completions.create") as mock_openai:
                ...
                with patch.dict(
                    ...
                    result = await perform_rag_query(
                        mock_mcp_context, query="test_function api/v2", match_count=5
                    )
