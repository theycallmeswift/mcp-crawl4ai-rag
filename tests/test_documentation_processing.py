...
class TestProcessDocumentFiles:
    ...
    def test_process_document_files_jupyter_notebook(self, temp_repo_dir, sample_notebook_content):
    ...
    def test_process_document_files_handles_encoding_errors(self, temp_repo_dir, mocker):
        ...
        mock_builtin_open.side_effect = UnicodeDecodeError("utf-8", b"", 0, 1, "invalid start byte")
...
class TestCreateDocumentationMetadata:
    ...
    def test_create_documentation_metadata_tutorial_detection(self, repo_info):
        ...
        doc_file_info = {"url": "docs/getting_started.md", "markdown": "# Getting Started"}
        result = create_documentation_metadata(doc_file_info, repo_info)
    ...
    def test_create_documentation_metadata_code_example_counting(self, repo_info, mocker):
        """Test code example counting when USE_AGENTIC_RAG=true."""
        # Set environment variable
        os.environ["USE_AGENTIC_RAG"] = "true"
        
        # Mock extract_code_blocks properly
        mock_extract_code_blocks = mocker.patch('utils.extract_code_blocks', return_value=['print("hello")', 'import os'])
...
class TestProcessRepositoryDocs:
    ...
    async def test_process_repository_docs_returns_correct_statistics(
        ...
        mock_chunk = mocker.patch('utils.smart_chunk_markdown', return_value=["chunk1", "chunk2", "chunk3"])
        mocker.patch('utils.add_documents_to_supabase', return_value=None)
